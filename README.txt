...
Make sure the following are installed on your machine:
  ...
  - MongoDB -- either:
      - A free MongoDB Atlas cluster (https://www.mongodb.com/cloud/atlas), or
...
2. Backend Setup
  ...
  Open backend/.env and fill in your values (see Environment Variables below).
...
3. Frontend Setup
  ...
  Open frontend/.env and set VITE_API_URL to your backend URL
...
Terminal 2 -- Frontend
  ...
  Note: The Vite dev server proxies all /api and /socket.io requests to
  http://localhost:5000 automatically -- no CORS issues in development.
...
ORGANIZATION & DATA ISOLATION
  ...
  - All API queries are scoped to organization: req.user.organization --
    users can never access data outside their own org
  - Organization names are globally unique -- no two organizations can share
    the same name
  - Email addresses are globally unique across the entire system
  - New members are created by an Admin from within the application --
    there is no self-registration path into an existing organization

================================================================================

SAMPLE CREDENTIALS

Use these pre-seeded accounts to explore the application. Both accounts belong
to the organization "Test Organization".
...
Admin Account
  ...
  Password   Admin@123
  Role       Admin
  Access     Full access -- manage users, view org stats,
             create/delete projects, manage all tasks
...
Member Account
  ...
  Password   User@123
  Role       Member
  Access     View and work on projects they are added to,
             manage their own tasks

Note: These accounts have been created in the deployed application.
